...
// el nombre es por el producto en categorias, la H es de la categoria de hombre y la F de su subcategoria de futbol
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const DATA = await fetchData(PRODUCTO_API, 'readProductosCategoriaHFutbol');
    // Se comprueba si la respuesta es satisfactoria, de lo contrario se muestra un mensaje con la excepción.
    if (DATA.status) {
        // Se asigna como título principal la categoría de los productos.
        MAIN_TITLE.textContent = `FootBall`;
