    ...
    if (isset($_SESSION['idAdmin'])) {
        ...
        switch ($_GET['action']) {
            ...
            case 'cantidadProductosCategoria':
                ...
                break;
            case 'mayorCantidadVentas':
                if ($result['dataset'] = $producto->mayorCantidadVentas()) {
                    $result['status'] = 1;
                } else {
                    $result['error'] = 'No hay datos disponibles';
                }
            ...
            case 'clientesFrecuentes':
                if ($result['dataset'] = $producto->clientesFrecuentes()) {
                    $result['status'] = 1;
                } else {
                    $result['error'] = 'No hay datos disponibles';
                }
                break;
            case 'estadoPedidoGrafico':
                if ($result['dataset'] = $producto->estadoPedidoGrafico()) {
                    $result['status'] = 1;
                } else {
                    $result['error'] = 'No hay datos disponibles';
                }
                break;
            case 'valoracionProductosGrafico':
                if ($result['dataset'] = $producto->valoracionProductosGrafico()) {
                    $result['status'] = 1;
                } else {
                    $result['error'] = 'No hay datos disponibles';
                }
                break;
            case 'ingresosMensualesGrafico':
                if ($result['dataset'] = $producto->ingresosMensualesGrafico()) {
                    $result['status'] = 1;
                } else {
                    $result['error'] = 'No hay datos disponibles';
                }
