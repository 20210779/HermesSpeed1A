    ...
    if (isset($_SESSION['idAdmin'])) {
        ...
        switch ($_GET['action']) {
            ...
            case 'createRow':
                ...
                // Definir las opciones válidas para el estado del pedido
                $estadoPedidoOptions = array('atrasado', 'en_reparto', 'ausente', 'reembolso');
                if (
                    !$pedido->setEstado($_POST['estadoPedido'], $estadoPedidoOptions) or
                    !$pedido->setFecha($_POST['fechaPedido']) or
                    !$pedido->setDireccion($_POST['direccioPedido']) or
                    !$producto->setCliente($_POST['clientePedido'])
                ) {
            ...
            case 'updateRow':
                ...
                if (!$pedido->setEstado($_POST['estadoPedido'], $estadoPedidoOptions)) {
                    $result['error'] = $pedido->getDataError();
